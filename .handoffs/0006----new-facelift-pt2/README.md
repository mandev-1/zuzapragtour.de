...
### 5. Journal index (`prototypes/` SPA `#/journal` — facelifted + ad-ready)
Page-banner "Geschichten aus *Prag*." with a **byline trust row** under the intro: 42px round Zuzana portrait + "Persönlich geschrieben von **Ing. Zuzana Manová**" + a computed "{count} Beiträge · Zuletzt erschienen {newest month}" — keep it computed from the dataset. **Featured lead** (newest article): real `<a href>` with aria-label + focus ring; 16:11 image in the brand's **brass offset frame** (1px brass, inset 14px/-14px) with `--shadow-md` and slow zoom; gold "Neuester Beitrag" capsule + category eyebrow (both `nowrap`); Italiana H2 up to ~3.4rem; the article's **standfirst in Cormorant italic** (~1.25rem) instead of the card blurb; clock-icon meta line (1rem, nowrap); explicit "Artikel lesen →" underline CTA (nowrap). Then a 3-col post grid — every card a real `<a href>` (aria-label, focus ring): 3:2 image, category (11.5px), Italiana H3 1.55rem, blurb 1.02rem ink-soft, and a bottom row with clock-icon meta + "Weiterlesen →" link. Dark CTA band: cream outline "Kontakt aufnehmen" + large phone link.

**Advertising (prepared inventory).** The journal is ad-ready, German-market compliant — every slot carries the legally required **"ANZEIGE"** microlabel (Trennungsgebot), 10px/600/0.3em uppercase in ink-mute, centered above the creative:
- **Billboard** after the featured article: hairline-ruled full-width band on `--canvas-alt`, creative centered at **970×250** (max-width 970, `aspect-ratio:970/250`, fluid below).
- **Native in-grid slot** at grid cell 5: styled like a journal card (hairline border, `--canvas-alt`, radius-lg), 3:2 creative area, plus a house line "Ihre Werbung im Journal — Mediadaten anfragen" (mailto).
- A **`showAds` boolean** (root prop / feature flag, default true) gates all inventory in one switch; when off, the grid closes up with no gaps (the ad item is simply not spliced in).
- In the prototype the creatives are drag-and-drop `<image-slot>` placeholders (ids `ad-journal-billboard`, `ad-journal-native`) so the client can preview real motifs; in production replace them with ad-server tags (e.g. Google Ad Manager) at the same geometry, keeping the ANZEIGE label and the ratio-reserved space (no CLS).

### 6. Article (`prototypes/` SPA `#/article/<slug>` — leveled-up reading experience)
Header sits in the **same 1080px shell as the body, left-aligned over the prose column** (not centered): breadcrumb, kicker "Category · X Lesezeit", Italiana H1 (burgundy-italic emphasized word via `<em>`), Cormorant standfirst, byline (round portrait + name + "Zertifizierte Stadtführerin · Date"), meta facts (labels 11px uppercase, values 1rem). Full-width 16:9 hero figure + italic caption. Body is a **block renderer** (see Content model) in a 2-column layout (`minmax(0,1fr) 260px`, prose max 680px) with a **sticky rail**:
- **Scroll-spy "INHALT" TOC**: 2px ink top rule; numbered entries (`01`… Italiana numerals) with a 2px left spine — active section turns the spine, numeral and label **burgundy** (600 weight), tracked via IntersectionObserver (`rootMargin` ≈ `-12% 0 -72% 0`); items are real interactive elements (role=button, Enter/Space, 2px focus ring, 0.98rem).
- **"Auf einen Blick" facts card**: 11px header, rows 0.95rem (keys ink-soft, values 600 ink).
- **CTA block**: full-width burgundy "Unverbindlich anfragen", lock line "Kostenlos & ohne Verpflichtung", then "Lieber persönlich?" + 1.15rem phone link.
Plus a **3px burgundy reading-progress bar** fixed at the very top (above the header) and a **48px round back-to-top button** (paper, hairline, shadow) bottom-right that fades in after ~0.9 viewport of scroll. Foot CTA + "Weiterlesen im Journal" 3-card grid.
...
- **Production note on scrolling:** the prototype drives the progress bar/back-to-top from a polling loop and uses *instant* programmatic scrolls — workarounds for the design tool's sandboxed preview, which suppresses scroll events and smooth scrolling. In the real app use the standard implementations: passive `scroll` listeners and `behavior:'smooth'` for TOC jumps, back-to-top and the route-change scroll reset (respect `prefers-reduced-motion`).
...
**Success state:** 52px filled check, "Vielen Dank für Ihre Anfrage!" (2.1rem), 24h expectation at 1.15rem, and "Sie haben es eilig?" + phone. Cosmetic in the prototype; wire to email/CRM in production. Booking CTAs elsewhere **deep-link here with the tour preselected** (`selectedTour`). The left "Direkt mit Zuzana" column is **`position:sticky` (top ~100px)** so the portrait card, phone and contact details stay in view while the long form scrolls.
...
- **Article:** `activeToc` (current section id, set by the TOC IntersectionObserver and by TOC clicks). **Ads:** `showAds` flag gates the journal ad inventory.
...
- `prototypes/Zuza Prague Tours.dc.html` — SPA consolidation of the public site (hash routing, live Leaflet maps, premium home, facelifted Touren/Tour-Detail/Journal, ad slots, article reading features, senior-optimized Kontakt). **Current source of truth for UX.** Reference only.
- `prototypes/image-slot.js` — drag-and-drop image placeholder used by the journal ad slots (prototype-only; replace with ad-server tags in production).
...
7. Wire interactions (header transition, scroll-reveal, preview swap, scroll-spy TOC + reading progress + back-to-top with standard listeners/smooth scrolling, form success).
8. Implement the journal ad inventory (ANZEIGE labels, 970×250 billboard + native 3:2 slot, `showAds` flag, ratio-reserved containers) with your ad server.
9. Deploy public + admin as two Netlify sites under one domain, different subdomains.
