...
.zas-top{display:flex;align-items:flex-end;justify-content:space-between;gap:1.5rem;padding:1.5rem 2.2rem 1.25rem;border-bottom:1px solid var(--rule);background:#FCFBF8;flex-shrink:0}
...
/**
 * AdminShell — the chrome for the redaction (CMS): a dark ink sidebar with
 * brand, primary nav and the signed-in user, plus a sticky top bar exposing
 * a breadcrumb, page title and an actions slot. The scrollable body hosts
 * whatever view is active (dashboard, editor, …).
 */
          ...
          <div className="zas-head">
            {breadcrumb && (
              <button type="button" className="zas-crumb" onClick={onBreadcrumb}>
                <span className="material-symbols-outlined">arrow_back</span>{breadcrumb}
              </button>
            )}
            <h1 dangerouslySetInnerHTML={typeof title === 'string' ? { __html: title } : undefined}>
